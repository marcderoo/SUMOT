...
app, somme_frequences, get_daily_word = (
    getattr(app_, "app", None),
    getattr(app_, "somme_frequences", None),
    getattr(app_, "get_daily_word", None),
)
...
class TestMenuRoute(unittest.TestCase):
    ...
    def test_daily_endpoint(self):
        ...
        for _ in range(2):
            ...
            # Default values should be used for score and count
            self.assertIn(
                'let score = parseInt("0");', res_decoded[-1]
            )  # Default score
            self.assertIn(
                'let count = parseInt("1");', res_decoded[-1]
            )  # Default count

            real_word = [
                line
                for line in res_decoded[-1].split("\n")
                if line.strip().startswith('const real_word = "')
            ][0].split('"')[1]

            # Assert that a random word from 'dico' is in the
            # rendered content and that it's not the DEFAUT value
            self.assertTrue(
                any(word.upper() == real_word for word in dico)
    ...
    def test_empty_file_name(self):
        ...
        )  # Flask typically handles this as 404
    ...
    def test_valid_def_word(self, mock_get):
        ...
        with app.test_client() as client:
            ...
            self.assertEqual(
                response.data.decode(), "1Définition principale pour le mot."
            )
    ...
    def test_valid_word(self):
        ...
        self.assertAlmostEqual(
            somme_frequences("esi"), 0.1337 + 0.0931 + 0.0866, places=4
        )
    ...
    def test_easy_difficulty(self):
        ...
        response = self.app.post(
            "/ia/0", data=json.dumps(data), content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
        self.assertIn(
    ...
    def test_hard_difficulty(self):
        ...
        response = self.app.post(
            "/ia/3", data=json.dumps(data), content_type="application/json"
        )
