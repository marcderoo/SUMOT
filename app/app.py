...
def read_file_from_s3(key):
    ...
    try:
        ...
        try:
            ...
            print(
                f"Erreur lors de la lecture du fichier local {key.split('/')[-1]}: {local_e}"
            )
            return None
...
def get_daily_word():
    ...
    try:
        ...
        trends = [
            trend["name"]
            for trend in response_json[0]["trends"]
...
def solo() -> str:
    ...
    if request.method == "POST":
        ...
        count: int = request.form.get(
            "count", 1, type=int
        )  # Word count retrival from form

    random_word = random.choice(dico).upper()  # Real word
    return render_template(
        "solo.html", data={"word": random_word, "score": score, "count": count}
    )
...
def versus_ia() -> str:
    ...
    random_word = random.choice(dico).upper()  # Real word
    return render_template(
        "versusia.html", data={"word": random_word, "score": score, "count": count}
    )
...
def daily() -> str:
    global daily_word
    return render_template(
        "daily.html", data={"word": daily_word, "score": 0, "count": 1}
    )
...
def bot_proposition_difficile(difficulte: str) -> Optional[str]:
    ...
    data: Dict[
        str, Union[int, List[str], Dict[str, Dict[str, Union[int, bool, List[int]]]]]
    ] = request.get_json()
    words: List[str] = [
        word.upper()
        for word in dico
        if len(word) == data["len"]
        and word[0].upper() == data["firstLetter"]
        and word not in data["history"]
    ]
    ...
    for word in words:
        if difficulte > 0:
            ...
            for letter in word:
                ...
                if (
                    difficulte > 1
                    and letter in data["stateLetters"]
                    and countLetters[letter] > data["stateLetters"][letter]["count"]
                    and data["stateLetters"][letter]["notMore"]
                ):
                    goToNext = True
                    break
            if goToNext:
            ...
            for letter in data["stateLetters"].keys():
                if (
                    letter not in countLetters
                    and data["stateLetters"][letter]["count"] > 0
                ) or (
                    letter in countLetters
                    and data["stateLetters"][letter]["count"] > countLetters[letter]
                ):
                    goToNext = True
            if goToNext:
                continue
        ...
        for i in range(data["len"]):
            if (
                difficulte > 0
                and word[i] in data["stateLetters"]
                and i in data["stateLetters"][word[i]]["posGood"]
            ):
                break
...
def table() -> str:
    ...
    params.pop(
        "title", None
    )  # Supprimer le paramètre 'title' de la liste des paramètres
    ...
    return render_template(
        "table.html", data={"title": title, "content": json.dumps(data)}
    )
...
def fetch() -> str:
    ...
    raw_aggs = (
        request.args.get("aggs", "[]")
        .replace("%20", "")
        .replace("__DATEYEAR", datetime(datetime.now().year, 1, 1).isoformat())
        .replace("__DATEWEEK", (datetime.now() - timedelta(days=7)).isoformat())
    )
    print(raw_aggs)
...
def log_session():
    ...
    try:
        ...
        log = {"timestamp": datetime.now(timezone.utc), **data}
...
if __name__ == "__main__":
    ...
    scheduler.add_job(
        ...
        "interval",
        days=1,
        next_run_time=datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
        + timedelta(days=1),
