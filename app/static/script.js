                 ...
                 // Disable giving unvalid letter help if every unvalid letter as already been detected
                  ...
                  if(res.every(e => e === 2)){
                      ...
                      if(PLAYERTURN === -1 || attemps % 2 === PLAYERTURN){
                        score += actScore;
                        confetti.launch();
                        appUtils.emit("endGame", {score : actScore, attemps : attemps, count : count, success : true});
                      }
                      else {
                        appUtils.emit("endGame", {score : actScore, attemps : attemps, count : count, success : false});
                      }
                      appUtils.updateKey("score", score);
                    ...
                    if(cellBeforeFirstEmptyCell.index !== cells.length - 1){
                        ...
                        appUtils.emit("endGame", {score : actScore, attemps : Math.floor(cellBeforeFirstEmptyCell.index / NBLETTERS), count : count, success : false});
